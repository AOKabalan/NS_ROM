    ...
    for (Re, amp) in path:
        ...
        if prev_u_coeffs is None:
            ...
            u_coeffs, p_coeffs = project_to_rom_coefficients(
                src_vel, src_pres, operators, amp=amp, M_u=M_u, M_p=M_p)
        else:
        ...
        solution, w_rom, callback_data = solve_rom(
            operators=operators, nu=nu, amp=amp, problem=problem,
            deim_ops=deim_ops,
            u_initial_guess=u_coeffs, p_initial_guess=p_coeffs,
            submesh_deim=submesh_deim, return_internals=True)
        t_rom = time.perf_counter() - t0
        solution_rom = reconstruct_solution(solution, operators, problem, amp=amp)

        # --- indicator ---
        mu, rom_ind = float('nan'), None
        if solution.converged:
            rom_ind = compute_rom_indicator(
                w_rom, callback_data, operators, M_uu_red,
            ...
            if crossed:
                ...
                for s in (+1, -1):
                    res = in_sweep_branch_jump_rom(
                        ...
                        Re=Re, amp=amp, operators_k=operators, M_uu_red_k=M_uu_red,
                        problem=problem, eps=eps, sign=s, indicator=rom_ind,
                        offset=jump_offset, deim_ops=deim_ops,
                        submesh=submesh_deim)
        ...
        if verbose:
            ...
            print(f"[{branch_id}] Re={Re:.2f} amp={amp:.2f}  {status} "
                  f"it={solution.iterations} mu={mu:.2e}")

        record = {'branch': branch_id, 'Re': Re, 'amp': amp,
        ...
        # --- history update ---
        prev_u_coeffs, prev_p_coeffs = solution.velocity_coeffs, solution.pressure_coeffs
...
# ---------------------------------------------------------------------------
def _three_branches(
    path, operators, deim_ops, submesh_deim,
    problem, initial_solution, M_u, M_p, M_uu_red,
    *, eps=0.8, jump_offset=0, lock_children=False, fom_compute=False, verbose=True,
    ...
    for sp in spawns:
        ...
        seed = {
            ...
            'amp':      amp_c,
            'solution_rom': reconstruct_solution(sp['sol'], operators, problem, amp=amp_c),
        }


        child_records, _, child_final, _ = _continue(
            child_path, operators, deim_ops, submesh_deim,
            problem, initial_solution, M_u, M_p, M_uu_red,
...
# public entry point
# ---------------------------------------------------------------------------
def build_full_global_diagram_bare(
    Re_range, amp_values,
    operators, deim_ops,
    problem, initial_solution, M_u, M_p, M_uu_red,
    use_deim=True,
    *,
    ...
    base_records, anchors = _three_branches(
        path0, operators, deim_ops, submesh_deim,
        problem, initial_solution, M_u, M_p, M_uu_red,
        eps=eps, jump_offset=jump_offset,
        fom_compute=fom_compute, verbose=verbose)
    ...
    for base_id, anchor in anchors:
        for side_name, amp_list in (("-", neg), ("+", pos)):
            ...
            spine_records, _, _, spine_states = _continue(
                spine_path, operators, deim_ops, submesh_deim,
                problem, initial_solution, M_u, M_p, M_uu_red,
            ...
            for st in spine_states:
                ...
                sweep_records, _, _, _ = _continue(
                    redown_path,operators, deim_ops, submesh_deim,
                    problem, initial_solution, M_u, M_p, M_uu_red,
