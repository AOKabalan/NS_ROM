#!/usr/bin/env python
"""
FOM pitchfork bifurcation diagram for the fluidic pinball.
...
Pipeline (FOM only):
  ...
  5. plot a publication-quality C_L(Re) bifurcation diagram
...
# Publication palette (consistent with plot_critical_curve.py).
C_POS = "#1f4e79"   # C_L > 0 branch
C_NEG = "#b3541e"   # C_L < 0 branch
C_SYM = "0.45"      # symmetric branch

SINGLE_COL = 3.40   # inches
DOUBLE_COL = 7.00
FIG_WIDTH = SINGLE_COL


# =============================================================================
# STYLE  (matches plot_critical_curve.py)
# =============================================================================
...
def plot_bifurcation(sym_records, pos_diag, neg_diag, Re_c, width,
                     pos_mirror=False, neg_mirror=False):
    fig, ax = plt.subplots(figsize=(width, 0.82 * width))

    # Computed symmetric branch, split at Re_c by stability.
    Re_s, CL_s = diag_arrays(sym_records)
    (Re_lo, CL_lo), (Re_hi, CL_hi) = split_at(Re_s, CL_s, Re_c)
    ...
    # Asymmetric branches.
    Re_p, CL_p = diag_arrays(pos_diag)
    Re_n, CL_n = diag_arrays(neg_diag)

    if Re_p.size:
        lab = r"asymmetric, $C_L > 0$" + (" (reflected)" if pos_mirror else "")
        ax.plot(Re_p, CL_p, "-o", color=C_POS, ms=3, mew=0, lw=1.2,
                zorder=3, label=lab)

    if Re_n.size:
        lab = r"asymmetric, $C_L < 0$" + (" (reflected)" if neg_mirror else "")
...
def main():
    ...
    if pos_diag and not neg_diag:
        ...
        print("\n  WARNING: C_L<0 branch not found by solver; "
              "plotting reflection of C_L>0 branch.")
    ...
    fig = plot_bifurcation(
        sym_records, pos_diag, neg_diag, Re_c, FIG_WIDTH,
        pos_mirror=pos_mirror, neg_mirror=neg_mirror,
    )

    for ext in ("pdf", "png"):
        f = os.path.join(OUTDIR, f"bifurcation_diagram.{ext}")
        fig.savefig(f)
        print(f"  wrote {f}")
    ...
    npz_path = os.path.join(OUTDIR, "bifurcation_diagram.npz")
    np.savez(
        npz_path,
        Re_c=Re_c,
        mu_c=mu_c,
