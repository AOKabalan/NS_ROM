...
def center_snapshot_matrix(matrix):
    mean_snapshot = matrix.mean(axis=1, keepdims=True)
    centered = matrix - mean_snapshot
    return centered

def normalize_snapshot_matrix(matrix, M_u):
    n_snapshots = matrix.shape[1]
    energy_norms = np.zeros(n_snapshots)

    # Compute energy norms: ||u||_M = sqrt(u^T M u)
    for i in range(n_snapshots):
        u = matrix[:, i]
        # Both sparse and dense @ operator work efficiently
        Mu = M_u @ u
        energy_norms[i] = np.sqrt(np.dot(u, Mu))
    # Normalize each snapshot
    normalized = matrix / energy_norms[np.newaxis, :]
...
def main():
    ...
    branch_ids = np.array(snapshot_data['branch_ids'])
    ...
    # before POD I can center and normalize the snapshots
    S_centered = center_snapshot_matrix(S)

    # --- POD ---
    section("POD COMPUTATION")

    modes_centered, eigenvalues_centered = compute_pod_modes(S_centered, M_u, n_modes=POD_RANK)
    modes, eigenvalues = compute_pod_modes(S, M_u, n_modes=POD_RANK)

    A = project_to_basis(S, modes, M_u)
    A_centered = project_to_basis(S_centered, modes_centered, M_u)
    print(f' shape of a {A.shape}')
    print(f' normal A : {A[0,:]}')
    print(f' centered A : {A_centered[:,:3]}')
    fig, axes = plt.subplots(2, 4, figsize=(16, 6), sharex=True)
    for i in range(4):
        axes[0, i].scatter(range(n_snap), A[i, :], c=branch_ids, cmap='tab10', s=8)
        axes[0, i].set_title(f'Mode {i+1} (raw)')
        axes[1, i].scatter(range(n_snap), A_centered[i, :], c=branch_ids, cmap='tab10', s=8)
    ...
    # --- Proxy clusters vs ground truth ---
    section("BRANCH LABELS: ground truth vs Mahalanobis proxy")

    # Match proxy cluster count to actual branch count
    ...
    print("  Ground truth branches (from snapshot data):")
    for b in unique_branches:
        mask = branch_ids == b
        print(f"    Branch {b}: {np.sum(mask)} snapshots, "
    ...
    # Agreement between proxy and ground truth
    # (using best permutation matching)
    ...
    for perm in permutations(range(n_proxy)):
        ...
        # Compare with branch_ids (may have different number of labels)
    ...
    # Color map for branches (2 or 3 branches)
    n_branches = len(np.unique(branch_ids))
    branch_colors = ['#2196F3', '#F44336', '#4CAF50']  # blue, red, green
    branch_labels = [f'Branch {i}' for i in range(n_branches)]

    modes_per_fig = 2

    for fig_idx in range(0, POD_RANK, modes_per_fig):
        mode_indices = list(range(fig_idx, min(fig_idx + modes_per_fig, POD_RANK)))
        n_modes_this_fig = len(mode_indices)

        fig, axes = plt.subplots(1, n_modes_this_fig, figsize=(12 * n_modes_this_fig, 7))
        if n_modes_this_fig == 1:
            axes = [axes]  # ensure iterable

        for plot_pos, idx in enumerate(mode_indices):
            ax = axes[plot_pos]

            for b in range(n_branches):
                mask = np.array(branch_ids) == b
                ax.scatter(np.where(mask)[0],
                        A[idx, mask],
            ...
            ax.set_title(f'Mode {idx + 1}', fontsize=13, fontweight='semibold', pad=8)
            ax.set_xlabel('Spatial/Temporal Index', fontsize=11, fontweight='medium')
            ax.set_ylabel('Amplitude', fontsize=11, fontweight='medium')
            ...
            ax.axhline(y=0, color='k', linewidth=0.4, alpha=0.2)

        # Shared legend on each figure
        handles = [
            plt.Line2D([0], [0], marker='o', color='w',
                    markerfacecolor=branch_colors[b], markersize=9,
                    label=branch_labels[b])
            for b in range(n_branches)
        ]
        fig.legend(handles=handles, title='Branch', fontsize=10,
                title_fontsize=10, loc='upper right',
                bbox_to_anchor=(1.0, 1.02), framealpha=0.9)

        mode_range_str = f'Modes {mode_indices[0] + 1}–{mode_indices[-1] + 1}'
        fig.suptitle(f'POD Modes — {mode_range_str}',
    ...
    # Color map for distinct professional lines (kept for subplot title styling)
    ...
    for idx in range(POD_RANK):
        ...
        # --- Scatter each point colored by its branch_id ---
        for b in range(n_branches):
            mask = np.array(branch_ids) == b
            ax.scatter(np.where(mask)[0],          # x: original indices
                    A[idx, mask],               # y: amplitude at those indices
                    color=branch_colors[b],
                    label=branch_labels[b],
                    s=8,
                    alpha=0.8,
    ...
    # --- Shared legend (drawn once on the figure level) ---
    handles = [
        plt.Line2D([0], [0], marker='o', color='w',
                markerfacecolor=branch_colors[b], markersize=7,
                label=branch_labels[b])
        for b in range(n_branches)
    ]
    fig.legend(handles=handles, title='Branch', fontsize=9,
    ...
    # Use ground-truth Fisher for all downstream analysis
    ...
    # Branch coloring for scatter plots
    branch_cmap = {0: '#0F6E56', 1: '#D85A30', 2: '#185FA5',
                   3: '#534AB7', 4: '#993556'}
    branch_scatter_colors = [branch_cmap.get(b, '#888') for b in branch_ids]
    ...
    ax.set_title('Parameter space by branch')
    ax.legend(fontsize=7)
    ...
    ax.set_title('C_L vs Amplitude by branch')
    ax.axhline(0, color='gray', lw=0.5, ls='--')
    ax.legend(fontsize=7)
    ...
    ax.set_xlabel('POD mode'); ax.set_ylabel('Fisher J (true branches)')
    ax.set_title('Branch separation'); ax.legend(fontsize=7)
    ...
    # Pick one smooth, one bifurcation, one mixed mode
    ...
    if bif_indices:
        ...
        if np.mean(bif_ranks) > np.mean(smooth_ranks) if smooth_ranks else True:
